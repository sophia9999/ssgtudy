            ...
            <table class="table table-lg" id="table1">
                <thead>
                    <tr class="text-center">
                        ...
                        <th class="col-md-3">멤버 닉네임</th>
                        <th class="col-auto">멤버 역할</th>
                        <th class="col-md-4">변경</th>
                    </tr>
                </thead>
                <tbody>
                	<c:forEach var="dto" items="${list}">
                		<tr class="text-center">
							...
							<td>
								<div class="btn-group">
									<select name="role" class="select form-select memberRole" style="width: 70%">
										<option value="20">스터디장</option>
										<option value="10">스터디관리자</option>
										<option value="1">일반멤버</option>
										<option value="0">대기멤버</option>
									</select><button type="button" class="btn btn-primary changeRole" value="${dto.memberNum}"> <!-- onclick="changeRole('${dto.memberNum}')" -->변경</button>
								</div>
							</td>
                		</tr>
                	</c:forEach>
...
$(function name() {
	$("body").on("click", ".changeRole", function() {
		...
		var nickName = $(this).closest("tr").find("td:eq(1)").html();

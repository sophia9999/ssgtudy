				...
				<div class="tab-pane fade show active mt-3" id="nav-content" role="tabpanel" aria-labelledby="nav-tab-content">
							...
							<td colspan="2" align="left">
								<c:choose>
									<c:when test="${menuItem=='receive'}">
										보낸사람 : ${dto.senderNickName}(${dto.senderId})
									</c:when>
									<c:when test="${menuItem=='send'}">
										받는사람 : ${dto.receiverNickName}(${dto.receiverId})
									</c:when>
								</c:choose>
							</td>
						</tr>
						...
						<tr>
							<td colspan="2" class="ellipsis">
								...
								<c:if test="${not empty preDto}">
									<span><a href="${pageContext.request.contextPath}/note/${menuItem}/noteArticle?${query}&noteNum=${preDto.noteNum}">${preDto.content}</a></span>
								</c:if>
							</td>
						</tr>
						<tr>
							<td colspan="2" class="ellipsis">
								...
								<c:if test="${not empty nextDto}">
									<span><a href="${pageContext.request.contextPath}/note/${menuItem}/noteArticle?${query}&noteNum=${nextDto.noteNum}">${nextDto.content}</a></span>
								</c:if>
							</td>
						</tr>						
					...
					<table class="table table-borderless">
						<tr>
							...
							<td class="text-end">
								<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/note/${menuItem}/noteForm?${query}';">리스트</button>
							</td>
						</tr>
					</table>
